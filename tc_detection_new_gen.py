...
class tc_tracks(object):
    ...
    def set_thresholds(self,thr_wind,thr_mslp,p_radius,neighborhood_size,warm_core_size,cores_distance,min_time_steps):
    ...
    def find_closed_contours(self,field,y,x,step=2,search_radius=30,n_contours=None):
        '''
    ...
    def detect(self,overwrite=False):
        ...
        for t,progress in zip(self._time_i,np.array([['-']+['']*(len(self._time_i)/20+1)]*20).flatten()[0:len(self._time_i)]):
            ...
            # i vort max
            y_p_mins,x_p_mins = self.local_min(self._MSLP_smoothed[t,:,:],threshold=self._thr_mslp,neighborhood_size=self._neighborhood_size)
            for y_p,x_p in zip(y_p_mins,x_p_mins):
                tc_area,ncont=self.find_closed_contours(self._MSLP_smoothed[t,:,:],y_p,x_p,step=2,search_radius=self._p_radius)
                if ncont>0:
                    ...
                    warm_core_area,ncont=self.find_closed_contours(self._T[t,1,:,:],y_t,x_t,step=-1,search_radius=self._p_radius,n_contours=1)
                    if len(np.where(warm_core_area==1)[0])<self._warm_core_size:
                        tmp[4]=1
...
def date_(t):
    ...
    return(datetime.datetime(yr,mn,day,hr,0))
...
found_tcs.set_thresholds(thr_wind=15,thr_mslp=101500,p_radius=27,neighborhood_size=10,warm_core_size=3,cores_distance=1,min_time_steps=10)

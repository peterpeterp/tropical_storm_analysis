    ...
    class tc_tracks(object):
        ...
        def plot_surrounding(self,win1=7,win2=4,thr_wind=13,thr_sst=26):
            for t,t_i in zip(self.time[self.time_i],self.time_i):
                ...
                im=ax.imshow(self.ta.values[t_i,1,:,:],vmin=255,vmax=275)
                im.set_cmap('bone')
                ax.autoscale(False)
                ax.axis('off')
                # add ibtracks info
                ...
                for y,x in zip(y_p,x_p):
                    for y_,x_ in zip(y_v,x_v):
                        ...
                        if x_<x+win2 and x_>x-win2 and y_<y+win2 and y_>y-win2:
                            ...
                            tmp_t=self.ta.ix[t_i,:,:,:].mean(axis=0)
                            tmp_t_sec=tmp_t.ix[max(0,y-win1):min(y+win1,len(self.lat)-1),max(x-win1,0):min(x+win1,len(self.lon)-1)]
                            if tmp_t_sec.max()-tmp_t_sec.mean()>2:
                                y_t=np.where(tmp_t==tmp_t_sec.max())[0][0]
                                x_t=np.where(tmp_t==tmp_t_sec.max())[1][0]
                                print x_t,x,y_t,y
                                if x_t<x+win2 and x_t>x-win2 and y_t<y+win2 and y_t>y-win2:
                                    ax.plot([x-win1*1.2,x-win1*1.2,x+win1*1.2,x+win1*1.2,x-win1*1.2],[y-win1*1.2,y+win1*1.2,y+win1*1.2,y-win1*1.2,y-win1*1.2],'--g')

                plt.suptitle(str(dates[t_i]))
                plt.tight_layout()
                plt.savefig(self.working_dir+str(t_i)+'.png', bbox_inches = 'tight')

        def plot_detected(self,out_name=None):
            if out_name is None:
                out_name=self.working_dir+'track_'+str(self.year)+'_found_locations.png'

            for storm in range(len(self.tc_sel.storm)):
                tc_plot(self.m,self.tc_lon[storm,:],self.tc_lat[storm,:],self.tc_intens.values[storm,:,0])

            for dec in self.detected:
                self.m.plot(dec[2],dec[1],marker='*',color='m')

            for track in self.tracks.values():
                self.m.plot(track[:,2],track[:,1],'-*c')
        ...
        def combine_tracks(self):
            tmp_detected=self.detected[:]
            self.tracks={}
            id_=0
            for loc in tmp_detected:
                self.tracks[id_]=[loc]
                tmp_detected.remove(loc)
                for loc2 in tmp_detected:
                    if loc[0]-loc2[0]<50:
                        if ((loc[1]-loc2[1])**2+(loc[2]-loc2[2])**2)**0.5<5:
                            self.tracks[id_].append(loc2)
                            tmp_detected.remove(loc2)

                self.tracks[id_]=np.array(self.tracks[id_])
                id_+=1

        def complete_track(self,t,y_ini,x_ini,plot):
            t_i=np.where(self.time==t)[0][0]
            print t,t_i
        ...
        def detect(self,win1=9,win2=6,thr_wind=13,thr_sst=26,plot=True):
            self.detected=[]
            self.detected_i=[]
            for t,t_i in zip(self.time[self.time_i],self.time_i):
            ...
            for dec in detected:
                print dec
                self.complete_track(dec[0],dec[1],dec[2],plot)
    ...
    for year in [2008]:
        ...
        found_tracks[year]=tc_tracks(wind,mslp,sst,vort,ta,dates,year,tc_sel,working_dir=working_dir,time_steps=range(90,120))
        #found_tracks[year].detect_TC(wind_thresh=15,sst_thresh=26,plot=True)
        #found_tracks[year].detect(thr_wind=10)
        #found_tracks[year].combine_tracks()
        #found_tracks[year].plot_detected()

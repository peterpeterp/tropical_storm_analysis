...
obs_stats=obs_stats[np.isfinite(obs_stats[:,'obs_pres']),:]

obs_stats_norm=da.DimArray(obs_stats.copy(),axes=[range(obs_stats.shape[0]),obs_tracks.variable],dims=['id','variable'])
obs_tracks_norm=obs_tracks.copy()
for var in obs_stats.variable:
    tmp=obs_stats[:,var].copy()
    obs_stats_norm[:,var]=(tmp-tmp.mean())/tmp.std()
    obs_tracks_norm[:,:,var]=(obs_tracks[:,:,var]-tmp.mean())/tmp.std()
...
plt.tight_layout()
plt.savefig('detection/JRA55/JRA55_obs_norm.png')

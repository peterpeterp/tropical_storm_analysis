...
class tc_tracks(object):
    ...
    def combine_tracks(self,plot=True,overwrite=False):
        ...
        care about warm core only in the end
            ...
            if p not in used_pos:
                ...
                while True:
                    ...
                    for p_1 in postions[postions[:,0]==p[0]-1,:].tolist():
                        if ((p[1]+y_mo-p_1[1])**2+(p[2]+x_mo-p_1[2])**2)**0.5<self._search_radius:
                ...
                if sum([pp in used_pos for pp in track])/float(len(track))<0.3:
                    ...
                    if track.shape[0]>=12:
                        if track[track[:,'warm_core']==1].shape[0]>=8:
                            warm_strong=track[(track[:,'Wind10']>=self._thr_wind) & (track[:,'warm_core']==1)]
                            consecutive=np.diff(warm_strong[:,'t'],1)==1
                            consec_info=consecutive_sequence(consecutive)
                            print('**************')
                            print(warm_strong[:,'t'].values)
                            print(consec_info)
                            if max(consec_info[:,1])>=6:
                                first_of_consec=consec_info[np.argmax(consec_info[:,1]),0]
                                print(first_of_consec)
                                if self._lats[int(warm_strong.ix[first_of_consec,1]),int(warm_strong.ix[first_of_consec,2])]<40:
                                    print(self._lats[int(warm_strong.ix[first_of_consec,1]),int(warm_strong.ix[first_of_consec,2])])
    ...
    def plot_detect_summary(self,out_name=None):
        ...
        tmp.append(self.plot_on_map(self._ax,detect[:,'x'],detect[:,'y'],linestyle='',marker='o',c='c'))
        warm_core=detect[detect[:,'warm_core']==1]
        tmp.append(self.plot_on_map(self._ax,warm_core[:,'x'],warm_core[:,'y'],linestyle='',marker='*',c='r'))
        strong_wind=detect[detect[:,'Wind10']>=self._thr_wind]
        tmp.append(self.plot_on_map(self._ax,strong_wind[:,'x'],strong_wind[:,'y'],linestyle='',marker='^',c='m'))
...
found_tcs.set_thresholds(thr_wind=17.5,thr_mslp=101500,p_radius=27,neighborhood_size=3,warm_core_size=3,cores_distance=1,search_radius=6,min_time_steps=10)

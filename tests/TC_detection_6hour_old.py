...
class tc_tracks(object):
    ...
    def set_thresholds(self,thr_wind,thr_sst,thr_vort,thr_mslp,thr_ta,win1,win2,win3,neighborhood_size):
    ...
    def save_track(self,track,t_i,plot=True):
        if track.shape[0]>4:
            if self.sst[track[0,0],self.lat[np.argmin(abs(self.lat-track[0,1]))],self.lon[np.argmin(abs(self.lon-track[0,2]))]]>self.thr_sst:
                self.tcs[str(self.id_)]=da.DimArray(track,axes=[track[:,0],['time','lat','lon']],dims=['time','pos'])
        ...
        for p in points.tolist():
            if p not in used_pos:
                ...
                while True:
                    ...
                    candidates=[]
                    for p_1 in points[points[:,0]==p[0]+6,:].tolist():
        ...
        for t,t_i in zip(self.time[self.time_i],self.time_i):
            ...
            for y,x in zip(y_v,x_v):
                ...
                if sum([(yy,xx) in p_mins for yy,xx in surrounding]):
                    detect[t_i][(y,x)].append('mslp')

                    # iii wind speed
                    if self.wind.ix[t_i,y_win2,x_win2].max()>self.thr_wind:
                        detect[t_i][(y,x)].append('wind')
                        # iv warm core
                        if self.ta.ix[t_i,y_win1,x_win1].max()-self.ta.ix[t_i,y_win2,x_win2].mean()>self.thr_ta:
                            detect[t_i][(y,x)].append('ta')
...
for year in range(2008,2009):
    ...
    found_tracks[year].set_thresholds(thr_wind=12,thr_vort=1*10**(-4),thr_mslp=101500,thr_ta=0,thr_sst=26,win1=3,win2=5,win3=7,neighborhood_size=4)
